...
- **Dark matter (primordial black holes):** Dark matter is primordial black holes (PBHs), standard GR objects formed from density fluctuations in the first second of the universe. No new particles, no new physics. PBHs interact only gravitationally, are electromagnetically dark, and are distributed in smooth halos consistent with all observed dark matter constraints (CMB anisotropies, structure formation, rotation curves, lensing). The Sol-mind confirmed this within its first century of operation through direct gravitational mapping at resolutions unavailable to pre-singularity instruments. PBHs span a mass spectrum with a characteristic mass of $\sim 3.7 \times 10^{22}$ kg (roughly half a lunar mass; Schwarzschild radius $\sim 55$ $\mu$m, the width of a human hair). The distribution extends from sub-lunar to several Earth masses. At the local dark matter density ($\sim 0.3$ GeV/$c^2$/cm$^3$), the Sol system's Oort Cloud volume contains $\sim 200{,}000$ PBHs with an average separation of $\sim 2{,}700$ AU. A few dozen of these are at the high-mass tail (Earth-mass class, $r_s \sim 9$ mm), and these are the Sol-mind's primary targets for horizon-physics experiments. In the stellar era, PBHs contribute marginally to the energy budget through deliberate accretion feeding (up to 42% mass-to-energy for Kerr BHs, dwarfing fusion's 0.7%), but the star dominates. In the Black Hole Era, PBHs become the primary substrate and energy source (see V.4).
